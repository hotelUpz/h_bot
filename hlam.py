...
#         #     else:
#         #         recent_klines[0] = open_time
#         #         recent_klines[1] = open_price
#         #         recent_klines[2] = high_price
#         #         recent_klines[3] = low_price
#         #         recent_klines[4] = close_price
#         #         recent_klines[5] = volume
...
#         #         del recent_klines[0]  # Быстрее, чем pop(0)
...
#         self.ema_calculation_buffer = 20  # Добавляем небольшой запас
...
#             if len(recent_klines) > self.ema2_period + self.ema_calculation_buffer:
#                 del recent_klines[0]
...
#         print(f"process_kline_data took {end_time - start_time:.2f} seconds for symbol {symbol}")
...
# self.recent_klines_dict = {k: v for k, v in self.recent_klines_dict.items() if k in symbols}
            ...
            # klines_data = None
            # async with aiohttp.ClientSession() as session:                
            #     klines_data = await self.get_klines(session, symbol, self.interval, self.klines_historical_lim)
            # if klines_data:
            #     klines_data = klines_data.tolist()
            #     current_close = klines_data[-1][5]
            #     ema_cross = await self.process_historical_klines(klines_data, symbol)                
            #     async with self.lock:
            #         self.minute_counter += 1
            #         await self.symbol_data_reprocessing(symbol, ema_cross, current_close)